...
7. 撤销对账（重新对账法）
   适用场景：已完成对账后发现漏记条目（如信用卡还款），需作废该次对账并重新执行。
   用户 → 前端（待办列表 → 对账 → 历史，点击「撤销对账」）
       → 确认对话框
       → 后端（POST /reconciliation/tasks/{id}/revoke_reconciliation/）
       → 基于内容匹配注释 trans/reconciliation.bean 中当次条目
       → 数据库（原任务 status 更新为 revoked）
       → 更新或新建同账户 pending 待办（scheduled_date 为当天）
       → 用户可对同一日期重新执行对账
```
...
- [x] 撤销对账：已完成的对账任务可撤销，账本中当次指令被注释，允许对同一日期重新对账 [completion:: 2026-03-06]
