...
1. 由于**记录数量太多**，若每个记录都以单独条目记录则需要耗费大量时间，若以天为条目进行记账，又会导致条目的颗粒度太大；
...
3. 当出现多个条目解析优先级相同时：[payee](#解析格式) 字段会保留原始商家
...
默认会对支付宝支付状态为 " 退款成功 "、" 交易关闭 "、" 解冻成功 "、" 信用服务使用成功 "、" 已关闭 "、" 等待付款 " 、" 芝麻免押下单成功 " 的条目进行忽略。
...
捐赠收入将全部用于提高 [网站](https://trans.dhr2333.cn/) 访问速度，并支持公众号 "TC 听云智能 "。
